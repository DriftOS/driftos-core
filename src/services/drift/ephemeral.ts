...
function mergeFacts(
  ...
  for (const fact of newFacts) {
    ...
    if (fact.isUpdate && existingValues) {
      ...
      console.log(`[MERGE_FACTS] Updating fact key "${fact.key}" with ${fact.values.length} values (messageId: ${messageId})`);
      console.log(`[MERGE_FACTS] Existing values:`, existingValues.map(ev => ({ value: ev.value, messageId: ev.messageId, status: ev.status })));
      ...
      for (const newValue of fact.values) {
        ...
        if (newValue.supersedes && newValue.supersedes.length > 0) {
          for (const supersededValue of newValue.supersedes) {
            ...
            if (targetIndex !== -1) {
              ...
              console.log(`[MERGE_FACTS] Superseding "${supersededValue}" with "${newValue.value}"`);
              existingValues[targetIndex]!.status = 'superseded';
              existingValues[targetIndex]!.supersededBy = messageId;
            } else {
              console.log(`[MERGE_FACTS] ⚠️ LLM tried to supersede "${supersededValue}" but it doesn't exist or isn't active (hallucination)`);
            }
          }
        }
        ...
        if (!isDuplicate) {
          ...
          console.log(`[MERGE_FACTS] ⚠️ Skipping duplicate value "${newValue.value}" for key "${fact.key}"`);
        }
      }

      console.log(`[MERGE_FACTS] After merge:`, existingValues.map(ev => ({ value: ev.value, messageId: ev.messageId, status: ev.status })));
    } else {
      // New fact key or no existing values
      console.log(`[MERGE_FACTS] Creating new fact key "${fact.key}" with ${fact.values.length} values`);
...
export async function processEphemeralConversation(
  ...
  for (let i = 0; i < messagesToProcess.length; i++) {
    ...
    console.log(`[EPHEMERAL] Processing message ${messageIndex} role=${msg.role}, content="${msg.content.slice(0, 50)}..."`);
    ...
    const ctx: DriftContext = {
      ...
      requestId: messageId,
      startTime: Date.now(),
      perfTracker: { recordStageStart: () => {}, recordStageEnd: () => {} } as any,
      results: {},
      errors: [],
      metadata: {},
    };
    ...
    if (classification.action === 'BRANCH') {
      ...
      const factsRecord: Record<string, Array<{
        value: string;
        messageId: string;
        confidence: number;
        status: 'active' | 'superseded' | 'removed';
